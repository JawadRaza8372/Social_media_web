...
function PosterUserWthBtn({ userinfo, userId, curentUser, notif }) {
  ...
  let messageRoom = async () => {
    await db
      ...
      .add({
        member1: curentUser,
        member2: userId,
        roomdate: new Date().toDateString(),
        roomtime: new Date().toLocaleTimeString(),
      })
      .then(() => {
        notif();
      });
  ...
  let fetchdata = async () => {
    await db
      ...
      .doc(curentUser)
      .onSnapshot((doc) => {
        setcurentuserData(doc.data().following);
      });
  };
  ...
  let followfunction = async () => {
    ...
    await db
      .collection("users")
      .doc(userId)
      .update({ followers: userfollowers })
      .then(() => {
        db.collection("users")
          .doc(curentUser)
          .update({ following: curentuserData })
          .then(() => {
            setbuttontype("2");
          });
      });
      ...
      >
        ...
        <div className="col-xs-10 col-sm-6 col-md-4 col-lg-3 col-xl-3 mb-2 ml-2">
          {buttontype === "1" ? (
            ...
            <IconButton onClick={messageRoom}>
